...
def e_transposition(plaintext):
    ...
    wordList = [x for x in plaintext if ord(x) in range(ord('a'),ord('z')+1) or x == ' ']

    #determines size of square
    n=1
    while len(wordList) > n**2:
        n+=1
    ...
    wordKey = []
    x=0
    while len(wordKey) < len(wordList):
        if x < len(wordList):
            wordKey.append(x)
            x+=n
        else:
    ...
    newWord = []
    for x in wordKey:
        newWord.append(wordList[x])
    return "".join(newWord)

#decode
def d_transposition(ciphertext):
    ciphertext = ciphertext.lower()
    wordList = list(ciphertext)
    ...
    for x in wordKey:
        newWord.append(wordList[x])
    newWord = [x for x in newWord if ord(x) in range(ord('a'), ord('z')+1) or x==" "]
    return "".join(newWord)
...
def e_substitution(plaintext, keyword):
    ...
    for x in range(0,len(keyword)):
        if keyword[x] not in keyList:
            keyList.append(ord(keyword[x]))
    for x in range(ord('a'),ord('z')+1):
        if x not in keyList:
            keyList.append(x)
    legend = dict(zip(range(ord('a'),ord('z')+1),keyList))
    plaintextList = [ord(x) for x in list(plaintext)]
    newWord = []
    for x in plaintextList:
        try:
            newWord.append(chr(legend[x]))
        except:
            newWord.append(chr(x))
    return "".join(newWord)
#decode
def d_substitution(plaintext, keyword):
    ...
    for x in range(ord('a'),ord('z')+1):
        if x not in keyList:
            keyList.append(x)
    legend = dict(zip(keyList, range(ord('a'),ord('z')+1)))
    plaintextList = [ord(x) for x in list(plaintext)]
    newWord = []
    for x in plaintextList:
        try:
...
radioSub = Radiobutton(frm2, text="Substitution", value=5, variable = choice, command= radioUpdate)
radioShift = Radiobutton(frm, text="Shift", value=4, variable = choice, command= radioUpdate)
radioKey = Radiobutton(frm, text="Keyword", value=3, variable = choice, command= radioUpdate)
radioFlip = Radiobutton(frm, text="Flip", value=2, variable = choice, command= radioUpdate)
radioTransposition = Radiobutton(frm2, text="Square", value=1, variable = choice, command= radioUpdate)
